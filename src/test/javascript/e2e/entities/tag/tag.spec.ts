...
describe('Tag e2e test', () => {
    ...
    it('should load Tags', async () => {
        ...
        expect(await tagComponentsPage.getTitle()).to.eq('lwtApp.tag.home.title');
    });

    it('should load create Tag page', async () => {
        await tagComponentsPage.clickOnCreateButton();
        tagUpdatePage = new TagUpdatePage();
        expect(await tagUpdatePage.getPageTitle()).to.eq('lwtApp.tag.home.createOrEditLabel');
    ...
    it('should delete last Tag', async () => {
        ...
        expect(await tagDeleteDialog.getDialogTitle()).to.eq('lwtApp.tag.delete.question');
